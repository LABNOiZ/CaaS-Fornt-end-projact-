<template>
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-1">
                ...
                <div class="flex gap-1">
                    <button @click="currentRoleFilter = 'ALL'" :class="currentRoleFilter === 'ALL' ? 'bg-gray-800 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'" class="px-3 py-1 rounded-md text-[10px] font-bold transition-all">All</button>
    ...
    <div class="flex-1 min-h-0 flex flex-col overflow-hidden relative mb-2">
        <div class="flex-none py-2 flex justify-between items-center mb-1">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent relative rounded-xl shadow-sm border border-gray-100">
            <table class="w-full text-left border-collapse table-fixed"> 
                <thead class="bg-white text-gray-500 font-semibold uppercase tracking-wider text-[10px] sticky top-0 z-10 shadow-sm border-b border-gray-200">
                    <tr>
                        <th class="px-4 py-2.5 w-[20%] bg-white backdrop-blur-sm border-b border-gray-100">ชื่อ-นามสกุล</th>
                        <th class="px-4 py-2.5 w-[20%] bg-white backdrop-blur-sm border-b border-gray-100">Email</th>
                        <th class="px-4 py-2.5 w-[12%] bg-white backdrop-blur-sm border-b border-gray-100">Role</th>
                        <th class="px-4 py-2.5 w-[8%] bg-white backdrop-blur-sm border-b border-gray-100 text-center">Branch</th>
                        <th class="px-4 py-2.5 w-[10%] bg-white backdrop-blur-sm border-b border-gray-100 text-center">Status</th>
                        <th class="px-4 py-2.5 w-[15%] bg-white backdrop-blur-sm border-b border-gray-100">Created By</th>
                        <th class="px-4 py-2.5 w-[12%] bg-white backdrop-blur-sm border-b border-gray-100">Date</th>
                        <th class="px-4 py-2.5 w-[9%] bg-white backdrop-blur-sm border-b border-gray-100 text-right pr-6">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 bg-white">
                    <tr v-for="(user, index) in filteredUsers" :key="user.id || index" class="hover:bg-blue-50/30 transition duration-150 group">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div class="flex flex-col min-w-0"> 
                                    <div class="flex items-center gap-1">
                                        <span class="font-bold text-gray-800 text-[11px] truncate select-text">{{ user.displayFullNameTh }}</span>
                                        <button v-if="user.displayFullNameTh !== '-'" @click="copyToClipboard(user.displayFullNameTh)" class="text-gray-400 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition relative outline-none" title="คัดลอก">
                                            <DocumentDuplicateIcon class="w-3.5 h-3.5" />
                                            <span v-if="copiedText === user.displayFullNameTh" class="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[9px] px-1.5 py-0.5 rounded shadow-md whitespace-nowrap z-50">คัดลอกแล้ว!</span>
                                        </button>
                                    </div>
                                    <span class="text-[9px] text-gray-400 font-mono truncate select-text">{{ user.displayFullNameEn }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-2.5 align-middle text-[11px] text-gray-600 font-medium truncate select-text">{{ user.email }}</td>
                        ...
                        <td class="px-4 py-2.5 align-middle text-right pr-4">
                            <div class="flex justify-end gap-1">
                                <button @click="openReset2FAModal(user)" :disabled="user.status === 'SUSPENDED' || user.is2faEnabled === false" class="p-1 rounded hover:bg-orange-50 text-orange-400 hover:text-orange-600 transition disabled:opacity-30 disabled:cursor-not-allowed" title="Reset 2FA"><ShieldExclamationIcon class="w-4 h-4" /></button>
                    ...
                    <tr v-if="filteredUsers.length === 0 && !isLoading">
                        <td colspan="8" class="h-64 text-center bg-white">
...
const filteredUsers = computed(() => {
    ...
    return result.sort((a, b) => {
        ...
        // ถ้าสถานะเหมือนกัน (ACTIVE ทั้งคู่ หรือ INACTIVE ทั้งคู่): ให้เรียงตาม ก-ฮ
        const nameA = a.displayFullNameTh || ''
        const nameB = b.displayFullNameTh || ''
...
const mapUsersData = (data) => {
  ...
  users.value = data.map(u => {
    ...
    let enName = u.fullNameEn || u.fullNameEng || u.fullName_en || ''
    if (!enName && u.firstName && u.lastName) {
