<template>
  ...
  <div
    ...
    class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] animate-fade-in px-4"
  >
    <div
      class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 animate-bounce-in border border-gray-200"
    >
      <div
        class="bg-orange-50 p-6 flex flex-col items-center justify-center border-b border-orange-100"
      >
        <div
          class="h-16 w-16 rounded-full bg-orange-100 flex items-center justify-center mb-2"
        >
      ...
      <div class="p-6 text-center">
        ...
        <div class="text-3xl font-extrabold text-red-500 mb-6">
          {{ countdown }}
          <span class="text-sm font-normal text-gray-400">วินาที</span>
        </div>
...
// 2. กด อยู่ในระบบต่อ ปิด Modal และเริ่มนับเวลา Idle ใหม่โดยไม่ยิง API)
const handleStayConnected = () => {
  ...
  // เริ่มจับเวลาการไม่มีความเคลื่อนไหว
  resetTimer();
};
